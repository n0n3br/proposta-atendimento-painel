<template>
    <Fragment>
        ...
        <Contatos
            ...
            @favorite="favoritarContato"
        />
        <v-divider></v-divider>
        <v-row>
            <v-col :cols="colContratos" class="pt-0">
                <Contratos
            ...
            <v-col :cols="colHistoricoAtendimento" class="pt-0">
...
export default {
    ...
    created() {
        this.contratos = Array.from(Array(10)).map((_, i) => ({
            NU_CONTRATO: i,
            NU_CONTRATO_SIGA: `3920.110.${String(
                i + getRandom(0, 1000)
            ).padStart(6, "0")}-00`,
            IC_TIPO: "com",
            ...
            QT_ATRASO: getRandom(0, 1) ? getRandom(0, 365) : 0,
    ...
    watch: {
        contratosSelecionados() {
            if (!this.contratosSelecionados.length)
